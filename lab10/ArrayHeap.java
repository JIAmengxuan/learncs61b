...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        while(true) {
            int parentIndex = ArrayHeap.parentIndex(curIndex);
            if(!inBounds(parentIndex) || getNode(curIndex).priority() >= getNode(parentIndex).priority()) break;
    ...
    private void sink(int index) {
        ...
        int curIndex = index;
        while(true) {
            int leftIndex = ArrayHeap.leftIndex(curIndex);
            int rightIndex = ArrayHeap.rightIndex(curIndex);
            int childIndex;
            if(!inBounds(leftIndex) && !inBounds(rightIndex)) {
                break;
            } else {
                childIndex = min(leftIndex, rightIndex);
            }
            if(getNode(curIndex).priority() <= getNode(childIndex).priority()) break;
    ...
    public void changePriority(T item, double priority) {
        ...
        if(inBounds(flag)) {
            int parentNode = ArrayHeap.parentIndex(flag);
            if(inBounds(parentNode) && getNode(flag).priority() > getNode(parentNode).priority()) {
                swim(flag);
            } else {
                sink(flag);
            }
